...
String.prototype.removeStopWords = function() {
	...
	var stop_words = new Array('a', 'about', 'above', 'across', 'after',
			'again', 'against', 'all', 'almost', 'alone', 'along', 'already',
			'also', 'although', 'always', 'among', 'an', 'and', 'another',
	...
	for (x = 0; x < words.length; x++) {
		...
		for (y = 0; y < stop_words.length; y++) {
			...
			word = words[x].replace(/\s+|[^a-z]+/ig, ""); // Trim the word and
															// remove non-alpha
			...
			if (word.toLowerCase() == stop_word) {
				...
				regex_str += "|\\s+" + stop_word + "\\s+"; // Word somewhere in
															// the middle
